...
#### 编译 Rust 模块
```bash
cd packages/wasm-lang-test-rust
wasm-pack build --target bundler --release
```

#### 编译 Go 模块
```bash
cd packages/wasm-lang-test-go
GOOS=js GOARCH=wasm go build -o wasm_exec.wasm ./cmd/main.go
```

#### 编译 C++ 模块
```bash
cd packages/wasm-lang-test-cpp
emcripten src/main.cpp -o main.js -s WASM=1 -O3
```
...
2. 登录账户，进入 **Account Settings** → **Access Tokens**
